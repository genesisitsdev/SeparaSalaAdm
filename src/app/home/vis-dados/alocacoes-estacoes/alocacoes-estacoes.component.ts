...
@Component({
    selector: 'app-alocacoes-estacoes',
    templateUrl: './alocacoes-estacoes.component.html',
    styleUrls: [ './alocacoes-estacoes.component.css' ]
})
export class AlocacoesEstacoesComponent implements OnInit {
        ...
        this.alocacoesService.getAlocacoes().subscribe(
            (res) => {
                ...
                this.alocacoes = this.alocacoes.map((alocacao) => ({
                    ...alocacao,
                    empresasNome: alocacao.estacao.empresa.map((e) => e.nome).join(', ')
                }));
    ...
    clickDelete(alocacao) {
        if (confirm(`Deseja excluir a alocação ${alocacao.estacao.nome} ?`)) {
